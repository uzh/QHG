...
int qdf_readArray(hid_t hGroup, const char *pName, int iNum, double *pData) {
    ...
    herr_t status = H5Dread(hDataSet, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
int qdf_readArray(hid_t hGroup, const char *pName, int iNum, float *pData) {
    hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
    herr_t status = H5Dread(hDataSet, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
    qdf_closeDataSet(hDataSet);
    return (status >= 0)?0:-1;
}
...
int qdf_readArray(hid_t hGroup, const char *pName, int iNum, int *pData) {
    hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
    herr_t status = H5Dread(hDataSet, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
    qdf_closeDataSet(hDataSet);
    return (status >= 0)?0:-1;
}
...
int qdf_readArray(hid_t hGroup, const char *pName, int iNum, char *pData) {
    hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
    herr_t status = H5Dread(hDataSet, H5T_NATIVE_CHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
    qdf_closeDataSet(hDataSet);
    return (status >= 0)?0:-1;
}
