...
//
EventManager::~EventManager() {
    std::map<EventData *, Triggers *>::const_iterator it;
...
int EventManager::loadEventItem(EventData *pED, Triggers *pT, float fCurTime) {
    ...
    if ((pED != NULL) && (pT != NULL)) {
        std::map<EventData *, Triggers *>::iterator it;
        it =  m_vEDT.find(pED);
        if (it != m_vEDT.end()) {
...
void EventManager::triggerAll(float fTime) {
    ...
    for (it = m_vEDT.begin(); it != m_vEDT.end(); it++) {
        ...
        Triggers *pT = Triggers::createTriggers(sTrigger);
        it->second->merge(pT);
    }
}
...
double EventManager::findNextEvent() {
    double dMin=dPosInf;
    m_vEDSet.clear();
    // first find smallest time in all SeqIntervals
    std::map<EventData *, Triggers *>::const_iterator it;
    for (it = m_vEDT.begin(); it != m_vEDT.end(); it++) {
