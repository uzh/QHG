...
// constructor
//
PDAltPredPop::PDAltPredPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState) 
    : SPopulation<PDAltPredAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState),
      m_pIC(NULL),
      m_pWM(NULL),
      m_pME(NULL),
      m_pMM(NULL),
    ...
    ShareEvaluator<PDAltPredAgent> *pNSH0 = new ShareEvaluator<PDAltPredAgent>(this, pCG, NULL, "Prey0Mass");
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("ShareWeight0", pNSH0));

    // add altitude evaluation
    ShareEvaluator<PDAltPredAgent> *pNSH1 = new ShareEvaluator<PDAltPredAgent>(this, pCG, NULL, "Prey1Mass");
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("ShareWeight1", pNSH1));

    // add altitude evaluation
    SingleEvaluator<PDAltPredAgent> *pNSEGeo = new SingleEvaluator<PDAltPredAgent>(this, pCG, NULL, pAlt, "AltPreference");
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("AltWeight", pNSEGeo));

    m_pME   = new MultiEvaluator<PDAltPredAgent>(this, m_pCG, m_adEnvWeights, mEvalInfo);
...
#pragma omp parallel 
    ...
    m_prio.addAction(PDHUNTING_NAME, m_pPDHU);
