...
// constructor
//
PDAltPreyPop::PDAltPreyPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState) 
    : SPopulation<PDAltPreyAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState),
      m_afMassArray(NULL),
      m_pWM(NULL),
      m_pMM(NULL),
      m_pVer(NULL) {
    ...
    m_pSE = new SingleEvaluator<PDAltPreyAgent>(this, m_pCG, m_adEnvWeights, pAlt, parname);
...
int PDAltPreyPop::preLoop() {
    ...
    if (iResult == 0) {
        ...
        char s1[256];
        sprintf(s1, PD_TEMPLATE_INDEXES, m_sSpeciesName);
