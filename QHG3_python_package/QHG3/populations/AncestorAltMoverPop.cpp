...
// constructor
//
AncestorAltMoverPop::AncestorAltMoverPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState) 
    : SPopulation<AncestorAltMoverAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState),
    ...
    m_pAE = new SingleEvaluator<AncestorAltMoverAgent>(this, m_pCG, m_adEnvWeights, (double*)m_pCG->m_pGeography->m_adAltitude, m_pAltPrefName);
    

    m_pWM = new WeightedMove<AncestorAltMoverAgent>(this, m_pCG, m_apWELL, m_adEnvWeights);

    AncestorAltMoverAgent aama;
    m_pVer = new Verhulst<AncestorAltMoverAgent>(this, m_pCG, m_apWELL, (int)qoffsetof(aama, m_iMateIndex));

    m_pPair = new RandomPair<AncestorAltMoverAgent>(this, m_pCG, m_apWELL);

    m_pGO = new GetOld<AncestorAltMoverAgent>(this, m_pCG);

    m_prio.addAction(WEIGHTEDMOVE_NAME, m_pWM);
    m_prio.addAction(SINGLEEVAL_NAME, m_pAE);
...
// destructor
//
AncestorAltMoverPop::~AncestorAltMoverPop() {
...
// setParams
//
int AncestorAltMoverPop::setParams(const char *pParams) {
    int iResult = 0;
    ...
    printf("Have outdir [%s] and prefix[%s]\n", m_pOutDir, m_pPrefix);
    delete[] sParams;
    return iResult;
}
...
// preLoop
//
int AncestorAltMoverPop::preLoop() {
...
//    sprintf(m_sAncOutFileName,"pop_%d.anc",m_iSpeciesID);
    ...
    for (int iA = iFirstAgent; iA <= iLastAgent; iA++) {
        ...
        m_pAncBox->addBaby(m_aAgents[iA].m_ulID, 0, (m_aAgents[iA].m_iGender != 0)?-1:0);
    }
...
//
int AncestorAltMoverPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    
    // here we record parent info into the ancestor box

    AncestorAltMoverAgent* pBaby = &m_aAgents[iAgent];
    AncestorAltMoverAgent* pMother = &m_aAgents[iMother];
    AncestorAltMoverAgent* pFather = &m_aAgents[iFather];
        
    pBaby->m_fAge = 0.0;
    m_pAncBox->addBaby(pBaby->m_ulID, pMother->m_ulID, pFather->m_ulID*((pBaby->m_iGender == 0)?1:-1));
...
//
int AncestorAltMoverPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int AncestorAltMoverPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
...
//
void AncestorAltMoverPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {
