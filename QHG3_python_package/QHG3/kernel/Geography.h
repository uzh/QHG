...
public:
    ...
    geonumber  *m_adWater;      // presence of water (rivers in cell, coast in neighborhood etc)
    geonumber  *m_adAngles;     // orientation of direction to neighbors: 0->east, pi/2->north, pi->west, 3pi/2 south
                                // this must be calculated
    double     m_adQMapHeadData[6]; // for QMap headers
 ...
 protected:
    void writeAltQMap(float fTime, int iStep);
    void writeAltSnap(float fTime, int iStep, SCellGrid* pCG);
};
