...
const uint LIFE_STATE_GHOST    = 2; // halo presence
const uint LIFE_STATE_BIRTHING = 3; // will have a baby 
const uint LIFE_STATE_MOVED    = 4; // moved
...
public:
    ...
    SPopulation(SCellGrid *pCG, PopFinder *m_pPopFinder, int iLayerSize, IDGen **apIDGen, uint32_t *aulState);
    virtual ~SPopulation();
    ...
    virtual int updateEvent(int EventID, float fT) { return 0;};
    ...
    void updateTotal();  
	void checkLists() { this->m_pAgentController->checkLists(); };    
