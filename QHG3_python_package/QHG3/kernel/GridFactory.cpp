...
int GridFactory::setGrid(const char *pLine) {
    ...
    if (iResult == 0) {
        if (bFlat) {
            ...
            sm["W"] = sW;
            sm["H"] = sH;
            sm["LINKS"] = (iGridType==GRID_TYPE_FLAT6)?"6":"4";
            sm["PERIODIC"] = bPeriodic?"1":"0";

            char* sPTData = new char[256];
            sprintf(sPTData, "%d [Linear] %f %f %d",
                    PR_LINEAR, 0.0, 0.0, 0);
            sm[SURF_LTC_PROJ_TYPE] = sPTData;
...
//
 int GridFactory::setDensity(int iIndex, ValReader *pVRD) {
    ...
    for (uint i=0; i<m_iNumCells; i++) {
        m_pVeg->m_adMass[iIndex][i] = pVRD->getDValue(m_pGeo->m_adLongitude[i], m_pGeo->m_adLatitude[i]);
    }
...
//
 int GridFactory::setANPP(int iIndex, ValReader *pVRN) {
    ...
    for (uint i=0; i<m_iNumCells; i++) {
        m_pVeg->m_adANPP[iIndex][i] = pVRN->getDValue(m_pGeo->m_adLongitude[i], m_pGeo->m_adLatitude[i]);
    }
