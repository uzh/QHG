...
void QMapReader<T>::initialize(const char *pFileName, bool bPrepareArrays, bool bInterpolate) {
    ...
    */
    ...
    if (m_bSubRegion) {
        ...
        double dMinLon  = getNextGridPointLon(m_dExtrLonMin) - m_dDLon;
        double dMaxLon  = getNextGridPointLon(m_dExtrLonMax);
        double dMinLat  = getNextGridPointLat(m_dExtrLatMin) - m_dDLat;
        double dMaxLat  = getNextGridPointLat(m_dExtrLatMax);

        printf("--> [%f,%f]x[%f,%f]\n", dMinLon, dMaxLon, dMinLat, dMaxLat);

        m_iNRLon = (int)floor(round(1+(dMaxLon-dMinLon)/m_dDLon));
        m_iNRLat = (int)floor(round(1+(dMaxLat-dMinLat)/m_dDLat));
   
        m_iNumIgnoreItems = (int)floor(round(1+(dMinLat-m_dGridLatMin)/m_dDLat))*m_iNLon + (int)floor(round(1+(dMinLon-m_dGridLonMin)/m_dDLat));

        // now replace relevant stuff
        m_dGridLonMin = dMinLon;
        m_dGridLatMin = dMinLat;
        
        m_dDataLonMin = dMinLon;
        m_dDataLonMax = dMaxLon;
        m_dDataLatMin = dMinLat;
        m_dDataLatMax = dMaxLat;
    } else {
        m_iNRLon = m_iNLon;
        m_iNRLat = m_iNLat;
    }
