...
int CondWeightedMove<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult = qdf_extractAttribute(hSpeciesGroup, CONDWEIGHTEDMOVE_PROB_NAME, 1, &m_dMoveProb);
...
int CondWeightedMove<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult = qdf_insertAttribute(hSpeciesGroup, CONDWEIGHTEDMOVE_PROB_NAME, 1, &m_dMoveProb);
...
int CondWeightedMove<T>::tryReadParamLine(char *pLine) {
   ...
   iResult += this->readPopKeyVal(pLine, CONDWEIGHTEDMOVE_PROB_NAME, &m_dMoveProb);
