...
int Hunting<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_extractAttribute(hSpeciesGroup,  HUNTING_EFFICIENCY_NAME,  1, &m_dEfficiency);
    iResult += qdf_extractAttribute(hSpeciesGroup,  HUNTING_USABILITY_NAME,   1, &m_dUsability);
    iResult += qdf_extractSAttribute(hSpeciesGroup, HUNTING_PREYSPECIES_NAME, NAME_LEN, m_sPreyPopName);
...
int Hunting<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_insertAttribute(hSpeciesGroup,  HUNTING_EFFICIENCY_NAME,  1, &m_dEfficiency);
    iResult += qdf_insertAttribute(hSpeciesGroup,  HUNTING_USABILITY_NAME,   1, &m_dUsability);
    iResult += qdf_insertSAttribute(hSpeciesGroup, HUNTING_PREYSPECIES_NAME, m_sPreyPopName);
...
int Hunting<T>::tryReadParamLine(char *pLine) {
   ...
   iResult += this->readPopKeyVal(pLine, HUNTING_EFFICIENCY_NAME, &m_dEfficiency);
   iResult += this->readPopKeyVal(pLine, HUNTING_USABILITY_NAME,  &m_dUsability);
   char *pVal = readKeyString(pLine,HUNTING_PREYSPECIES_NAME, "=");
   if (pVal != NULL) {
