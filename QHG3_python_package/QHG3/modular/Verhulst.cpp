...
int Verhulst<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_extractAttribute(hSpeciesGroup, VERHULST_CAPACITY_NAME, 1, &m_dK);
...
int Verhulst<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_insertAttribute(hSpeciesGroup, VERHULST_CAPACITY_NAME, 1, &m_dK);
...
int Verhulst<T>::tryReadParamLine(char *pLine) {
    ...
    iResult += this->readPopKeyVal(pLine, VERHULST_CAPACITY_NAME, &m_dK);
