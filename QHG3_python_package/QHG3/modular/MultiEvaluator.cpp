...
//
template<typename T>
MultiEvaluator<T>::MultiEvaluator(SPopulation<T> *pPop, SCellGrid *pCG, double *adOutputWeights, evaluatorinfos &mEvaluators, int iMode, bool* pbUpdateNeeded, bool bDeleteEvaluators)
    : Action<T>(pPop,pCG),
...
int MultiEvaluator<T>::initialize(float fT) {
    ...
    if ((m_pbUpdateNeeded != NULL && *(m_pbUpdateNeeded)) || fT==0 ) {
...
int MultiEvaluator<T>::multiplySingleWeights(float fT) {
    ...
    memset(m_adOutputWeights, 1, iArrSize*sizeof(double));
