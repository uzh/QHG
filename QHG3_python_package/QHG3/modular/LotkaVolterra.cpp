...
int LotkaVolterra<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_extractAttribute(hSpeciesGroup, LOTKAVOLTERRA_SELFRATE_NAME, 1, &m_dSelfRate);
    iResult += qdf_extractAttribute(hSpeciesGroup, LOTKAVOLTERRA_MIXRATE_NAME, 1, &m_dMixRate);
    iResult += qdf_extractSAttribute(hSpeciesGroup, LOTKAVOLTERRA_OTHERPOP_NAME, NAME_LEN, m_sOtherPopname);
    iResult += qdf_extractAttribute(hSpeciesGroup, LOTKAVOLTERRA_K_NAME, 1, &m_dK);
...
int LotkaVolterra<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_insertAttribute(hSpeciesGroup, LOTKAVOLTERRA_SELFRATE_NAME, 1, &m_dSelfRate);
    iResult += qdf_insertAttribute(hSpeciesGroup, LOTKAVOLTERRA_MIXRATE_NAME, 1, &m_dMixRate);
    iResult += qdf_insertSAttribute(hSpeciesGroup, LOTKAVOLTERRA_OTHERPOP_NAME, m_sOtherPopname);
    iResult += qdf_insertAttribute(hSpeciesGroup, LOTKAVOLTERRA_K_NAME, 1, &m_dK);
...
int LotkaVolterra<T>::tryReadParamLine(char *pLine) {
   ...
   iResult += this->readPopKeyVal(pLine, LOTKAVOLTERRA_SELFRATE_NAME, &m_dSelfRate);
   iResult += this->readPopKeyVal(pLine, LOTKAVOLTERRA_MIXRATE_NAME, &m_dMixRate);
   char *pVal = readKeyString(pLine, LOTKAVOLTERRA_OTHERPOP_NAME, "=");
   if (pVal != NULL) {
   ...
   iResult += this->readPopKeyVal(pLine, LOTKAVOLTERRA_K_NAME, &m_dK);
