...
int PDHunting<T>::preLoop() {
    ...
    if (iResult == 0) {
        ...
        if ((iResult == 0) && (m_mRelations.size() > 0)) {
            ...
            relation::const_iterator it;
            for (it = m_mRelations.begin(); (iResult == 0) && (it != m_mRelations.end()); ++it) {
                m_pPreyEff[i].first  = it->first;
                m_pPreyEff[i].second = it->second.first;
                i++;
            }

            char s[256];
            sprintf(s, PD_TEMPLATE_PREY, m_sPredName.c_str());
...
int PDHunting<T>::operator()(int iAgentIndex, float fT) {
    ...
    char s[256];
    sprintf(s, PD_TEMPLATE_ASSMAP, this->m_pPop->getSpeciesName());
...
int PDHunting<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult += qdf_extractSAttribute(hSpeciesGroup, PDHUNTING_RELATIONS_NAME, NAME_LEN, m_sRelationInput);
...
int PDHunting<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    *sRelations = '\0';
    ...
    iResult += qdf_insertSAttribute(hSpeciesGroup, PDHUNTING_RELATIONS_NAME, sRelations);
...
int PDHunting<T>::tryReadParamLine(char *pLine) {
   ...
   int iResult = 0;
   char *pVal = readKeyString(pLine, PDHUNTING_RELATIONS_NAME, "=");
   if (pVal != NULL) {
