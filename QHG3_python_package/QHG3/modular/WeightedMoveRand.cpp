...
int WeightedMoveRand<T>::extractParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult = qdf_extractAttribute(hSpeciesGroup, WEIGHTEDMOVERAND_PROB_NAME, 1, &m_dMoveProb);
...
int WeightedMoveRand<T>::writeParamsQDF(hid_t hSpeciesGroup) {
    ...
    iResult = qdf_insertAttribute(hSpeciesGroup, WEIGHTEDMOVERAND_PROB_NAME, 1, &m_dMoveProb);
...
int WeightedMoveRand<T>::tryReadParamLine(char *pLine) {
   ...
   iResult += this->readPopKeyVal(pLine, WEIGHTEDMOVERAND_PROB_NAME, &m_dMoveProb);
