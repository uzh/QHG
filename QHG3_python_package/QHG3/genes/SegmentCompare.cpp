...
/* compile options
 -I ../common 
 -I/opt/visit-2.7.3/2.7.3/linux-x86_64/include/vtk/vtk-6.0/ 
*/
...
int getGenomeSize(const char *sQHGPop,  char *sPopName) {
    ...
    if (hFile >= 0) {
        ...
        if (hPops >= 0) {
            ...
            if (hSpc >= 0) {
                int iRes = qdf_extractAttribute(hSpc, GENETICS_GENOME_SIZE, 1, &iGenomeSize);
                if (iRes != 0) {
                    iGenomeSize = -1;
                }
                qdf_closeGroup(hSpc);
...
int getSamples(const char *sQDFGrid, const char *pQDFPop, const char *sLocFile, const char *pPopName,
               locdata &mLocData, loctimeids &mmvIDs, idagdatamap &mAgentData, bool bParallelSample,
               int *piNumAgents, idtype **ppIDs) {
    int iResult = -1;
    printf("--- creating IDSampler from [%s]\n", sQDFGrid);
    IDSampler *pIS = IDSampler::createInstance(NULL, sQDFGrid);
    if (pIS != NULL) {
        printf("--- getting samples from [%s]\n", pQDFPop);
        stringvec vQDFPop;
        vQDFPop.push_back(pQDFPop);
        iResult =  pIS->getSamples(vQDFPop, pPopName, sLocFile, mLocData, mmvIDs, mAgentData, bParallelSample);
        if (iResult == 0) {
            uint iTotal = 0;
            loctimeids::const_iterator it1;
            for (it1 = mmvIDs.begin(); it1 != mmvIDs.end(); ++it1) {
                std::map<float, std::vector<idtype> >::const_iterator it2;
                for (it2 = it1->second.begin(); it2 != it1->second.end(); ++it2) {
                    iTotal += (uint)it2->second.size();
                }
            }
            *piNumAgents = pIS->getNumAgents();
            *ppIDs = new idtype[*piNumAgents];
            idtype *pIDs = pIS->getIDs();
            memcpy(*ppIDs, pIDs, *piNumAgents*sizeof(idtype));

            printf("Sampled: Total %d ids\n", iTotal);fflush(stdout);
...
// compare
//
int compare(GenomeSegments *pGS, ulong *pGenome1, ulong *pGenome2, int iNumBlocks, double *pSegSum) {
                        ...
                        if (pGS != NULL) {
                            ...
                            if (pReferenceGenome != NULL) {
                                ...
                                if (iResult == 0) {
                                    ...
                                    loctimeids     mmvIDs;      // location name -> (time -> vector of sampled IDs)
                                    idagdatamap    mAgentData;  // agent ID -> (momID, dadID, gender, cellID,lon, lat)
                                    locdata        mLocData;    // location name -> (lon, lat, dist, num)
                                    bool bParallelSample = false; 
                                    int iNumAgents = 0;
                                    idtype *pIDs = NULL;
                                    iResult = getSamples(sQDFGrid, sQDFPop, sLocList, pPopName,
                                                         mLocData, mmvIDs, mAgentData, bParallelSample,
                                                         &iNumAgents, &pIDs);
                                
                                    if (iResult == 0) {
                                        printf("--- got samples\n");
                                        std::map<idtype, ulong *> mIDGen;
                                        iResult = readGenomes(sQDFPop, pPopName, iGenomeSize, iNumAgents, pIDs, mmvIDs, mIDGen);
                                        printf("--- have genome for ids: %zd\n", mIDGen.size());

                                        /*
                                        // show ids with their genomes
                                        std::map<idtype, ulong *>::const_iterator it;
                                        for (it = mIDGen.begin(); it != mIDGen.end(); ++it) {
                                            printf("%ld : ", it->first);
                                            GeneUtils::showGenome(it->second, iGenomeSize, SHOW_ALL);
                                            printf("     ");
                                            for (int j = 0; j < iNumBlocks; j++) {
                                                printf("%lu ", it->second[j]);
                                            }
                                            printf("\n");
                                        }
                                        printf("ref : ");
                                        GeneUtils::showGenome(pReferenceGenome, iGenomeSize, SHOW_ALL);
                                        */
                                        
                                        loctimeids::const_iterator itL;
                                        // loop through locs
                                        printf("Have %zd locations\n", mmvIDs.size());
                                        for (itL = mmvIDs.begin(); itL != mmvIDs.end(); ++itL) {
                                            printf("  %s\n", itL->first.c_str());
                                            // one segment um/avg per location
                                            double *pSegSum = new double[iNumSegments];
                                            memset(pSegSum, 0, iNumSegments*sizeof(double));
                                            int iC = 0;
                                            std::map<float, std::vector<idtype> >::const_iterator itT;
                                            for (itT = itL->second.begin(); itT != itL->second.end(); ++itT) {
                                                const std::vector<idtype>&v  = itT->second;
                                                printf("v.size: %zd\n", v.size());
                                                for (uint i = 0; i < v.size(); ++i) {
                                                    ulong *pGenome = mIDGen[v[i]];
                                        ...
                                        //!!!! clean up mIDGen
                                        std::map<idtype, ulong *>::iterator it1;
                                        for (it1 = mIDGen.begin(); it1 != mIDGen.end(); ++it1) {
                                            delete[] it1->second;
                                        }
                                        //!!!! clean up vSegSum
                                        for (uint i = 0; i < vSegSum.size(); i++) {
