...
#include "DynAncGraph.h"

/*
static void listIDSet(const char *pCaption, idset sIDs) {
    printf("%s ", pCaption);
...
// createInstance
//   
DynAncGraph *DynAncGraph::createInstance() {
    DynAncGraph *pDAG = new DynAncGraph();
    return pDAG;
}
...
// createInstance
//   
DynAncGraph *DynAncGraph::createInstance(const char *pAncData, ulong iBlockSize) {
    DynAncGraph *pDAG = new DynAncGraph();
    int iResult = pDAG->init(pAncData, iBlockSize);
    if (iResult != 0) {
        delete pDAG;
        pDAG = NULL;
    }
...
// createInstance
//   
DynAncGraph *DynAncGraph::createInstance(const char *pAncData, ulong iBlockSize, const char *pOracleFile, unsigned char *aChecksum, int iCSLen) {
    DynAncGraph *pDAG = new DynAncGraph();
    int iResult = pDAG->init(pAncData, iBlockSize, pOracleFile, aChecksum, iCSLen);
    if (iResult != 0) {
        delete pDAG;
        pDAG = NULL;
    }
...
// init
//   
int DynAncGraph::init(const char *pAncData, ulong iBlockSize) {
    int iResult = -1;
    m_iBlockSize = iBlockSize;
    m_aAncBuf = new idtype[3*m_iBlockSize];
    ...
    if (m_fAnc != NULL) {
        ...
        if ((m_lFileSize % 3) == 0) {
            ...
            printf("Anc file size %ld should be diviible by three...\n", m_lFileSize);
        }
    }
    return iResult;
}
...
// init
//   
int DynAncGraph::init(const char *pAncData, ulong iBlockSize, const char *pOracleFile, unsigned char *aChecksum, int iCSLen) {
    int iResult = -1;
    m_iBlockSize = iBlockSize;
    m_aAncBuf = new idtype[3*m_iBlockSize];
    m_fAnc = fopen(pAncData, "rb");
...
//
int DynAncGraph::findLowerBound(int iLo, int iHi, int iResolution, idtype iVal) {
    while ((iHi - iLo) > iResolution) {
        int iCur = (iHi + iLo)/2;
        if (m_aAncBuf[3*iCur] < iVal) {
            iLo = iCur;
        } else {
            iHi = iCur;
        }
...
//
int DynAncGraph::loadAncs(idset &sItems, std::map<idtype, std::pair<idtype,idtype> > &mAncs) {
    int iResult = 0;
    ...
    for (it = sItems.begin(); (iResult == 0) && (it != sItems.end()); ++it) {
        if (findAncestorNode(*it) == NULL) {
            ...
            if (ito != m_mOracle.end()) {
                ...
                printf("[DynAncGraph::loadAncs]no upper bound found for id %ld\n", *it);
                iResult = -1;
            }
        }
    }
    ...
    while ((iResult == 0) && (itB != sBlocks.end())) {
        ...
        size_t iRead = fread(m_aAncBuf, 3*sizeof(idtype), m_iBlockSize, m_fAnc);
        if (iRead > 0) {
	    ...
	    /* this seems to run slower than direct search
            ...
            */
            ...
            while (it != sItems.end() && (*it <= iID) && (i < iRead) && (iResult == 0)) {
                while ((i < iRead) && (m_aAncBuf[3*i] < *it)) {
                    i++;
                }

                if (m_aAncBuf[3*i] == *it) {
                    mAncs[m_aAncBuf[3*i]] = std::pair<idtype,idtype>(m_aAncBuf[3*i+1], m_aAncBuf[3*i+2]);
                    ++it;
                    ++i; 
...
// calcKillPoints
// 
int DynAncGraph::calcKillPoints(idset &sInitial, int iType, int iLatency) {
    int iResult = 0;
    uint iPrev = 0;
    ...
    if (iType > 0) {
        ...
        case 7:
            ...
            break;
        default:
            printf("Illegal backreach value: %d\n", iType);
            iResult = -1;
        }
...
//
int DynAncGraph::backReach(idset &sInitial) {
    int iResult = 0;
    int ig = 0;
    ...
    while ((sCurrent.size() > 0) || (mChildMap.size() > 0)) {
        ...
        idset_cit it2;
        std::map<idtype, std::pair<idtype,idtype> > mAncs;
        ...
        for (it2 = sCurrent.begin(); (iResult == 0) && (it2 != sCurrent.end()); ++it2) {
            ...
            idtype iMomID = mAncs[*it2].first;
            idtype iDadID = 0;
            if (iMomID != 0) {
                iDadID = (idtype) labs(mAncs[*it2].second);
            }
            ...
            if ((iMomID > 0)  && (iDadID > 0)) {
                ...
                printf("[backreach]Bad parents for %ld\n", *it2);
                iResult = -1;
            }
        }
...
//   of an "earlier" child. In this case the backreach is the index of the "earlier" generation.
//
int DynAncGraph::backReach2(idset &sInitial) {
    ...
    while ((iResult == 0) && (sCurrent.size() > 0))  {
        ...
        for (it2 = sCurrent.begin(); (iResult == 0) && (it2 != sCurrent.end()); ++it2) {
            ...
            if ((iMomID > 0)  && (iDadID > 0)) {
                ...
                printf("[backreach2]Bad parents for %ld\n", *it2);
                iResult = -1;
            }
        }
        /*printf("\n");*/ fflush(stdout);
...
//
int DynAncGraph::backReach3(idset &sInitial) {
    SystemInfo *pSys = SystemInfo::createInstance();
    ...
    while ((iResult == 0) && (asCurrent[iCur].size() > 0))  {
        ...
        for (it2 = asCurrent[iCur].begin(); (iResult == 0) && (it2 != asCurrent[iCur].end()); ++it2) {
            ...
            if ((iMomID > 0)  && (iDadID > 0)) {
                ...
                printf("[backreach3]Bad parents for %ld\n", *it2);
                iResult = -1;
            }
...
//   array of sets for current (to prevent copying of large sets)
//
int DynAncGraph::backReach7(idset &sInitial) {
    SystemInfo *pSys = SystemInfo::createInstance();
...
// 
int DynAncGraph::backReach4(idset &sInitial, int iLatency) {
    SystemInfo *pSys = SystemInfo::createInstance();
        ...
        if (pRG != NULL) {
            ...
            for (it2 = asCurrent[iCur].begin(); (iResult == 0) && (it2 != asCurrent[iCur].end()); ++it2) {
                ...
                if ((iMomID > 0)  && (iDadID > 0)) {
                    ...
                    printf("[backreach4]Bad parents for %ld\n", *it2);
                    iResult = -1;
                }
            }
            /*printf("\n"); */fflush(stdout);
            ...
            iResult = -1;
            printf("Couldn't build RGeneration\n");
        }
    }
...
//
int DynAncGraph::createGraph2(idset &sInitial, const char *pTempName, intset &sSavePoints) {
    SystemInfo *pSys = SystemInfo::createInstance();
        ...
        for (it2 = asCurrent[iCur].begin(); (iResult == 0) && (it2 != asCurrent[iCur].end()); ++it2) {
            ...
            if (pAN != NULL) {
                ...
                if ((iMomID > 0)  && (iDadID > 0)) {
                    ...
                    if (itg == m_mKillPoints.begin()) {
                        if (m_sSelected.find(iMomID) != m_sSelected.end()) {
                            ...
                            if (pAN2 != NULL) {
                                ...
                                printf("Couldn't set child %ld to M%ld\n", *it2, iMomID);
                            }
                        }
                        if (m_sSelected.find(iDadID) != m_sSelected.end()) {
                            ...
                            if (pAN2 != NULL) {
                                ...
                                printf("Couldn't set child %ld to D%ld\n", *it2, iDadID);
                            }
                        }
                    }
                    ...
                    printf("[createGraph]Bad parents for %ld\n", *it2);
                    iResult = -1;
                }
            } else {
                printf("No node for %ld\n", *it2);
                iResult = -1;
            }
        }
...
//
int DynAncGraph::createGraph3(idset &sInitial, const char *pTempName, intset &sSavePoints) {
    SystemInfo *pSys = SystemInfo::createInstance();
...
// uses sets directly from m_vGenerations (calculated in backReach3)
//
int DynAncGraph::createGraph7(idset &sInitial, const char *pTempName, intset &sSavePoints) {
    SystemInfo *pSys = SystemInfo::createInstance();
            ...
            if (pAN != NULL) {
                ...
                if ((iMomID > 0)  && (iDadID > 0)) {
                    ...
                    // these relationships must be set explicitly for the first generation.
                            ...
                            printf("no node for dad %ld of %ld\n", iDadID, *it2);
                        }
                    } else {
                        printf("no node for mom %ld of %ld\n", iMomID, *it2);
                    }
...
//
int DynAncGraph::createGraph4(idset &sInitial, const char *pTempName, intset &sSavePoints) {
    SystemInfo *pSys = SystemInfo::createInstance();
...
// createGraph
// 
int DynAncGraph::createGraph(idset &sInitial, const char *pTempName, intset &sSavePoints) {
    int iResult = 0;
    int iTempBufSize = 100000;
    int iSum = 0;
...
// 
int DynAncGraph::loadOracle(const char *pOracleFile, unsigned char *aChecksum, uint iCSLen) {
    int iResult = -1;
    ...
    if (fIn != NULL) {
        ...
        if ((iRead == iCSLen) && (memcmp(aChecksum, pCheckSum, iCSLen) == 0)) {
                ...
                if (iRead > 0) {
                    for (uint i = 0; i < iRead; ++i) {
                        ...
                        if (iPos < m_lFileSize) {
                            ...
                            printf("Bad Oracle file: position for id  %ld is %ld > file size %ld\n", m_aAncBuf[2*i], iPos, m_lFileSize);
                            iResult = -1;
                            m_mOracle.clear();
                        }
                    }
            ...
            printf("Checksum does not match. The Oracle is not for this Anc file\n");
            iResult = -1;
        }
        delete[] pCheckSum;
        fclose(fIn);
...
int DynAncGraph::createOracle() {
    ...
    if (m_fAnc != NULL) {
        ...
        // rewind file
        fseek(m_fAnc, 0, SEEK_SET);
        ulong iStep = 3*m_iBlockSize;
        ulong iFilePos = 3*m_iBlockSize;
        idtype iID[3];
        ulong iPrev = 0;
        printf("Filesize %ld\n", m_lFileSize);
        while ((iResult == 0) && (sizeof(idtype)*iFilePos < m_lFileSize)) {
            fseek(m_fAnc, sizeof(idtype)*(iFilePos-3), SEEK_SET);
            ulong iRead = fread(iID, sizeof(idtype), 3, m_fAnc);
            if (iRead == 3) {
        ...
        if (iResult == 0) {
            ...
            if (iPrev < m_lFileSize) {
                fseek(m_fAnc, -3*sizeof(idtype), SEEK_END);
...
// createNodesForIds
// 
int DynAncGraph::createNodesForIds(const idset &sCur) {
    int iResult = 0;
    ...
    while ((iResult == 0) && (itB != sBlocks.end())) {
        ...
        fseek(m_fAnc, iMin*sizeof(idtype), SEEK_SET);
        //        printf("Reading block for %d starting at %d (real %zd)\n", iID, iMin, iMin*sizeof(int));
        ulong iRead = fread(m_aAncBuf, 3*sizeof(idtype), m_iBlockSize, m_fAnc);
        if (iRead > 0) {
            uint i = 0;
...
//                        printf(" -> found in block for %d starting at %d\n", iID, iMin);
                        ...
                        AncestorNode *pAN = new AncestorNode(m_aAncBuf[3*i], m_aAncBuf[3*i+1], (m_aAncBuf[3*i+1]==0)?0:(idtype)labs(m_aAncBuf[3*i+2]));
                        pAN->m_iGender = (m_aAncBuf[3*i+2] < 0)?1:0;
                        m_mIndex[m_aAncBuf[3*i]] = pAN;
...
// 
int DynAncGraph::saveAndDeleteFragement(const char *pFileBody, int iGen, int iTempBufSize, const intset &sGenSets) {
    int iResult = 0;
...
// saveAndDeleteFragement
// 
int DynAncGraph::saveAndDeleteSet(const char *pFileName, int iBufSize, idset &sIDs) {
    int iResult = 0;
    ...
    if (pBW!= NULL) {
        ...
        for (it = sIDs.begin(); (iResult == 0) && (it != sIDs.end()); ++it) {
            ...
            if (itm != m_mIndex.end()) {
                ...
                printf("couldn't find node for id %ld\n", *it);
            }
        }
        if (iC !=  sIDs.size()) {
            printf("Wrote %d instead of %zd nodes - deleting [%s]\n", iC, sIDs.size(), pFileName);
            delete pBW;
            // delete file
            remove(pFileName);
            iResult = -1;
        ...
        iResult = -1;
        printf("Coundn't open outputfile [%s]\n", pFileName);
    }
...
//   NumIDs   : int
//
int DynAncGraph::saveBin(BufWriter *pBW, bool bWriteHeader, bool bWriteNodes, bool bWriteFooter) {
    int iResult = 0;
...
//   NumIDs   : int
//
int DynAncGraph::saveBin(const char *pFileName) {
    int iResult = -1;
...
int DynAncGraph::writeOracle(const char *pOutput, unsigned char *aChecksum, int iCSLen) {
    int iResult = -1;
    int iBufSize = 16636;
    ...
    if (pBW != NULL) {
        ...
        printf("Couldn't open [%s] for writiong\n", pOutput);
    }
