#ifndef __DYNANCGRAPHX_H__
#define __DYNANCGRAPHX_H__
...
typedef struct ancdataX {
    ...
    ancdataX(idtype iM, idtype iD, idtype iL) 
        : iMomID(iM),
          iDadID(iD),
          iLocal(iL) {
    };
    ancdataX(idtype iM, idtype iD) 
        : iMomID(iM),
          iDadID(iD),
          iLocal(-1) {
    };
...
class DynAncGraphX : public AncGraphBase {
public:    
    static DynAncGraphX *createInstance();
    static DynAncGraphX *createInstance(const char *pAncData, ulong iBlockSize, uint iAncSize);
    static DynAncGraphX *createInstance(const char *pAncData, ulong iBlockSize, uint iAncSize, const char *pOracleFile, unsigned char *aChecksum, int iCSLen);
   ...
   ~DynAncGraphX();
    ...
    int loadAncs(idset &sItems, std::map<idtype, ancdataX> &mAncs);

protected:
    DynAncGraphX();
    int init(const char *pAncData, ulong iBlockSize, uint iAncSize);
    int init(const char *pAncData, ulong iBlockSize, uint iAncSize, const char *pOracleFile, unsigned char *aChecksum, int iCSLen);
