...
int PreyDistributor::registerPredator(const char *pPredName) {
    ...
    char s1[256];
    sprintf(s1, ATTR_PD_TEMPLATE_PREY, pPredName);
    ...
    for (int iCell = 0;  iCell < m_iNumCells; iCell++) {
        ...
        for (it = m_mRelations.begin(); (iResult == 0) && (it != m_mRelations.end()); ++it) {
            ...
            for (it2 = it->second.begin(); (iResult == 0) && (it2 != it->second.end()); ++it2) {
                char s[256];
                sprintf(s, ATTR_PD_TEMPLATE_INDEXES, it2->first.c_str());
                if (iCell == 0) {
...
#pragma omp parallel for
        ...
        char s[512];
        sprintf(s, ATTR_PD_TEMPLATE_ASSMAP, itf->first.c_str());
...
#pragma omp parallel
    ...
    */
    ...
    for (it = m_mRelations.begin(); (iResult == 0) && (it != m_mRelations.end()); ++it) {
        ...
        char s[256];
        sprintf(s, ATTR_PD_TEMPLATE_INDEXES, it->first.c_str());
                ...
                if (v.back() > 0) { 
                    ...
                    for (uint iPreyIndex = 0; iPreyIndex < pPreyIdx[iCell].size(); iPreyIndex++) {
                        ...
                        if (fPreyRatio >= 0) {
                            ...
                            if (dR0 < fPreyRatio) {
                                char sPredName[256];
                                sprintf(sPredName, ATTR_PD_TEMPLATE_INDEXES, it->second[k].first.c_str());
