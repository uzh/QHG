...
// open
//
hid_t qdf_openFile(const char *pFileName, bool bRW) {
    int iResult = -1;
    ...
    if (iResult == 0) {
        hFile = H5Fopen(pFileName, bRW?H5F_ACC_RDWR:H5F_ACC_RDONLY, H5P_DEFAULT);
        if (hFile > 0) {
            ...
            printf("qdf_openFile: couldn't open [%s] in %s mode\n", pFileName, bRW?"RDWR":"RDONLY");
        }
    } else {
...
// qdf_exists
//
bool qdf_exists(hid_t hGroup, const char *pName) {
    return (H5Lexists(hGroup, pName, H5P_DEFAULT));
}
...
//    uchar
//
int qdf_insertAttribute(hid_t hLoc, const char *pName, int iNum, uchar *cValue) {
    int iResult = -1;
    hsize_t dims = iNum;
    hid_t hAttrSpace = H5Screate_simple (1, &dims, NULL);
    hid_t hAttribute = H5Acreate(hLoc, pName, 
                                 H5T_NATIVE_UCHAR, hAttrSpace, 
                                 H5P_DEFAULT, H5P_DEFAULT);

    herr_t status = H5Awrite (hAttribute, H5T_NATIVE_UCHAR, cValue);
...
int qdf_insertAttribute(hid_t hLoc, const char *pName, int iNum, uint *sValue) {
    ...
    hid_t hAttrSpace = H5Screate_simple (1, &dims, NULL);
    hid_t hAttribute = H5Acreate(hLoc, pName, 
                                 H5T_NATIVE_UINT32, hAttrSpace, 
                                 H5P_DEFAULT, H5P_DEFAULT);

    herr_t status = H5Awrite (hAttribute, H5T_NATIVE_UINT32, sValue);
    
    if (status == 0) {
        iResult = 0;
    } else {
        printf("write uint attribute err\n");
    } 
    qdf_closeDataSpace(hAttrSpace);
    qdf_closeAttribute(hAttribute);
...
//    ulongs
//
int qdf_insertAttribute(hid_t hLoc, const char *pName, int iNum, ulong *lValue) {
    int iResult = -1;
    hsize_t dims = iNum;
    hid_t hAttrSpace = H5Screate_simple (1, &dims, NULL);
    hid_t hAttribute = H5Acreate(hLoc, pName, 
    ...
    if (status == 0) {
        ...
        printf("write ulong attribute err\n");
    } 
    qdf_closeDataSpace(hAttrSpace);
    qdf_closeAttribute(hAttribute);
...
//    uchar
//
int qdf_extractAttribute(hid_t hLoc, const char *pName, uint iNum, uchar *cValue) {
    int iResult = -1;
    hsize_t dims[2];
    ...
    if (H5Aexists(hLoc, pName)) {
        ...
        if (((rank == 1) && (dims[0] == iNum)) ||
            ...
            herr_t status = H5Aread(hAttribute, H5T_NATIVE_UCHAR, cValue);
...
int qdf_extractAttribute(hid_t hLoc, const char *pName, uint iNum, uint *iValue) {
    ...
    if (H5Aexists(hLoc, pName)) {
        ...
        if (((rank == 1) && (dims[0] == iNum)) ||
            ...
            herr_t status = H5Aread(hAttribute, H5T_NATIVE_UINT32, iValue);
...
//    ulong
//
int qdf_extractAttribute(hid_t hLoc, const char *pName, uint iNum, ulong *lValue) {
    int iResult = -1;
    hsize_t dims[2];
...
//   char
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, char *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_CHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   uchar
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, uchar *pData) {
    herr_t status = H5P_DEFAULT;
    if (H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_UCHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   short
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, short *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_SHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   ushort
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, ushort *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_USHORT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   int
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, int *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   uint
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, uint *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_UINT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   long
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, long *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_LONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   ulong
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, ulong *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_ULONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   llong
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, llong *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_LLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   ullong
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, ullong *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   float
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, float *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   double
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, double *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
...
//   ldouble
//
int qdf_replaceArray(hid_t hGroup, const char *pName, int iNum, ldouble *pData) {
    herr_t status = H5P_DEFAULT;
    if (!H5Lexists(hGroup, pName, H5P_DEFAULT)) {
        status = qdf_writeArray(hGroup, pName, iNum, pData);
    } else {
        hid_t hDataSet   = H5Dopen(hGroup, pName, H5P_DEFAULT);
        status = H5Dwrite(hDataSet, H5T_NATIVE_LDOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, pData);
