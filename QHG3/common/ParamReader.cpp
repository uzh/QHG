...
OptionInfo::OptionInfo(std::string sOptionDef, void *pVariable)
    ...
    if (pos != std::string::npos) {
        ...
        if (m_bCorrect && (m_cType != 's') && (m_cType != 'S') && (m_cType != 'b')) {
            ...
            char *pEnd;
            double dDummy = strtod(m_sName.c_str(), &pEnd);
            if (*pEnd == '\0') {
                m_bCorrect = false;
            }         
            // don't want an unused variable warning
            dDummy += 0;
        }
    } else {
        m_bCorrect = false;
...
public:
   ...
   ~OptionList();
    ...
    bool allMandatorySet(vecstrings &vsMissing);
    ...
    void collectOptions(vecstrings &vsOptions);
    uint numOptions() { return (uint)m_MapOptions.size();};
    void getMandatoryList(vecstrings &vMand);
    void getUnknownList(vecstrings &vUnknown);
...
// allMandatorySet
//
bool OptionList::allMandatorySet(vecstrings &vsMissing) {
    bool bAllSet = true;
    OPTIONLIST::iterator iter;
    for (iter = m_MapOptions.begin();
         iter != m_MapOptions.end();
...
// getMandatoryList
//
void OptionList::getMandatoryList(vecstrings &vMand) {
    OPTIONLIST::iterator iter;
    for (iter = m_MapOptions.begin();
         iter != m_MapOptions.end();
         ++iter) {
...
// getUnknownList
//
void OptionList::getUnknownList(vecstrings &vUnknown) {
    OPTIONLIST::iterator iter;
    for (iter = m_MapOptions.begin();
         iter != m_MapOptions.end();
         ++iter) {
            ...
            if (iter->second != NULL) {
                if (iter->second->isSet()) {
                    ...
                    if ((iter->second->getType() != 'b') ||
                        ...
                        fprintf(fOut, "%s%s%s%s ", sSep, sQuote, sValue.c_str(), sQuote); fflush(fOut);
                    }
                    if (bLines) {
                        fprintf(fOut, "\n");
                    }
...
// collectOptions
//
void OptionList::collectOptions(vecstrings &vsOptions) {
    OPTIONLIST::iterator iter;
    for (iter = m_MapOptions.begin();
         iter != m_MapOptions.end();
         ++iter) {
...
// getMandatoryParams
//
uint ParamReader::getMandatoryParams(vecstrings &vMand) {
...
// getUnknownParams
//
uint ParamReader::getUnknownParams(vecstrings &vUnknown) {
...
// getFreeParams
//
uint ParamReader::getFreeParams(vecstrings &vFree) {
    vFree.clear();
...
// collectOptions
//
void ParamReader::collectOptions(vecstrings &vsOptions) {
