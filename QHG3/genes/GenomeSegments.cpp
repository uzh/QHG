...
// getMaskedSegment
//   
ulong *GenomeSegments::getMaskedSegment(const ulong *pFullGenome, int iSegmentNumber) {
...
int GenomeSegments::findSegmentForNuc(uint iNucPos) {
    ...
    for (uint i = 0; (iResult < 0) && (i < m_vSegments.size()); i++) {
        segment *ps = m_vSegments[i];
        uint iOffset = ps->m_iFirstBlock*GeneUtils::NUCSINBLOCK;
        if ((iOffset+ps->m_iStartNuc < iNucPos) && 
            (iNucPos < (iOffset+ps->m_iStartNuc+ps->m_iLengthNuc))) {
            iResult = i;
        }
    }
}
