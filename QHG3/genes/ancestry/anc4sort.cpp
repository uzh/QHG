...
#define DEF_CHUNK_SIZE (4*100000)
#define DEF_TEMP_NAME  "__temp_anc4sort"

typedef struct anc4data {
    idtype iMomID;
    idtype iDadID;
    idtype iLocal;
    anc4data() 
    ...
    anc4data(idtype iM, idtype iD, idtype iL) 
        : iMomID(iM),
          iDadID(iD),
          iLocal(iL) {
    };
...
void usage(char *pApp) {
    ...
    printf("  <curID> <momID> <dadID> <timeLoc>\n");
...
int sortAndWrite(long *pBuf, size_t iRead, int iCount, char *pTempFile) {
    int iResult = -1;
    std::map<long, anc4data > mAncData;
    for (uint i = 0; i < iRead; i++) {
        mAncData[pBuf[4*i]] = anc4data(pBuf[4*i+1], pBuf[4*i+2], pBuf[4*i+3]);
    }
    if (iRead == mAncData.size()) {
        ...
        if (fOut != NULL) {
            int i = 0;
            std::map<long, anc4data>::const_iterator it;
            for (it = mAncData.begin(); it != mAncData.end(); ++it) {
                pBuf[i]   = it->first;
                pBuf[i+1] = it->second.iMomID;
                pBuf[i+2] = it->second.iDadID;
                pBuf[i+3] = it->second.iLocal;
            //    printf("  %d", pBuf[i]);
		i+=4;
            }
            size_t iWritten = fwrite(pBuf, 4*sizeof(long), iRead, fOut);
            if (iWritten == iRead) {
                iResult = 0;
            }
            fclose(fOut);
...
int createSortedChunks(const char*pInput, long *piChunkSize, char *pTempFile) {
	...
	ldiv_t q = ldiv(lSize, 4*sizeof(long)*(*piChunkSize));
        printf("div(%ld,%zd)= %ld r%ld\n", lSize,  4*sizeof(long)*(*piChunkSize), q.quot, q.rem);
        if (q.quot < 511) {
            iNumFiles = q.quot;
            if (q.rem > 0) {
                iNumFiles++;
            ...
            iNumFiles = 512;
            long iCS = 1+lSize/(4*sizeof(long)*511);
            ldiv_t q1 = ldiv(lSize, 4*sizeof(long)*iCS);
            printf("div(%ld,%zd)= %ld r%ld\n", lSize,  4*sizeof(long)*iCS, q1.quot, q1.rem);
            *piChunkSize = iCS;
            iNumFiles = q1.quot;
            if (q1.rem > 0) {
                iNumFiles++;
        ...
        long *pBuf = new long[4*(*piChunkSize)];
        ...
        while ((iResult == 0) && !feof(fIn)) {
            ...
            size_t iRead = fread(pBuf, 4*sizeof(long), *piChunkSize, fIn);
            if (iRead > 0) {
                iResult = sortAndWrite(pBuf, iRead, iC, pTempFile);
                iC++;
                iTotBytes += 4*sizeof(long)*iRead;
            }
        }
        printf("\n");
...
int mergeChunkFiles(char *pTempFile, int iCount, long iBufSize, const char *pOutput) {
    ...
    if (pBW != NULL) {
        ...
        AncFileBuffers *pAFB = AncFileBuffers::createInstance(pTempFile, iCount, (int)iBufSize, 4);
        if (pAFB != NULL) {
	    int iC = 0;
            iResult = 0;
            const idtype *p = pAFB->getSmallestRecord();
            while (p != NULL) {
	    //printf("(%d %d %d)", p[0], p[1], p[2]);
                pBW->addChars((char *)p, 4*sizeof(long));
