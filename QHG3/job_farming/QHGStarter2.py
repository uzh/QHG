...
class QHGStarter2:
    ...
    def call_standard_QHG(self):
        ...
        num_proc  = self.find_map_value('num_procs')

        new_env = os.environ
        if num_proc != '':
            new_env['OMP_NUM_THREADS'] = num_proc
        #-- end if
        ...
        shutil.rmtree(out_main_temp, ignore_errors=True)
        if (out_main_real != ''):
            ...
            self.outlog("\n")
            self.outlog("starting %s:" % (qhg_main))
            ...
            if (iResult == 0):
                ...
                self.outlog("renamed output dir to [%s]" % out_main_real)
            else:
                self.outlog("%s failed with %d" % (qhg_main, iResult))
            #-- end if
        
        else:
            self.outlog("Couldn't find unused dir of the form '%s_N' with N<%d"% (pop_qdf, MAX_IDX))
        #-- end if
    #-- end def 
