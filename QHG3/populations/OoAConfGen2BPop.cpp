...
#include "OoAConfGen2BPop.h"


//----------------------------------------------------------------------------
// constructor
//
OoAConfGen2BPop::OoAConfGen2BPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoAConfGen2BAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pNPPCap = new NPPCapacity<OoAConfGen2BAgent>(this, pCG, m_apWELL, m_adCapacities, iCapacityStride); 
    ...
    SingleEvaluator<OoAConfGen2BAgent> *pSEAlt = new SingleEvaluator<OoAConfGen2BAgent>(this, pCG, NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    ...
    SingleEvaluator<OoAConfGen2BAgent> *pSECap = new SingleEvaluator<OoAConfGen2BAgent>(this, pCG, NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("NPPWeight", pSECap));

    m_pME = new MultiEvaluator<OoAConfGen2BAgent>(this, m_pCG, m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true); // true: delete evaluators

    OoAConfGen2BAgent agent;
    m_pVerVarK = new VerhulstVarK<OoAConfGen2BAgent>(this, m_pCG, m_apWELL, m_adCapacities, iCapacityStride, (int)qoffsetof(agent, m_iMateIndex));


    // evaluator for movement

    m_pWM = new WeightedMove<OoAConfGen2BAgent>(this, m_pCG, m_apWELL, m_adEnvWeights);

    m_pPair = new RandomPair<OoAConfGen2BAgent>(this, m_pCG, m_apWELL);

    m_pGO = new GetOld<OoAConfGen2BAgent>(this, m_pCG);

    m_pOAD = new OldAgeDeath<OoAConfGen2BAgent>(this, m_pCG, m_apWELL);

    m_pFert = new Fertility<OoAConfGen2BAgent>(this, m_pCG);

    m_pGenetics = new Genetics<OoAConfGen2BAgent,GeneUtils>(this, m_pCG, m_pAgentController, &m_vMergedDeadList, m_aiSeeds[1]);

    m_pNavigate = new Navigate<OoAConfGen2BAgent>(this, m_pCG, m_apWELL);

    m_pCM = new ConfinedMove<OoAConfGen2BAgent>(this, m_pCG, this->m_vMoveList);
...
// destructor
//
OoAConfGen2BPop::~OoAConfGen2BPop() {
...
// setParams
//
int OoAConfGen2BPop::setParams(const char *pParams) {
    int iResult = 0;
...
// preLoop
//
int OoAConfGen2BPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoAConfGen2BAgent>::preLoop();
...
//
int OoAConfGen2BPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoAConfGen2BPop::flushEvents(float fT) {
...
//
int OoAConfGen2BPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
...
//
int OoAConfGen2BPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoAConfGen2BPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void OoAConfGen2BPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {
...
//
int OoAConfGen2BPop::readAdditionalDataQDF(hid_t hSpeciesGroup) {
