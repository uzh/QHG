...
#include "OoANavGen2bitPop.h"


//----------------------------------------------------------------------------
// constructor
//
OoANavGen2bitPop::OoANavGen2bitPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavGen2bitAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pNPPCap = new NPPCapacity<OoANavGen2bitAgent>(this, pCG, m_apWELL, m_adCapacities, iCapacityStride); 
    ...
    SingleEvaluator<OoANavGen2bitAgent> *pSEAlt = new SingleEvaluator<OoANavGen2bitAgent>(this, pCG, NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    ...
    SingleEvaluator<OoANavGen2bitAgent> *pSECap = new SingleEvaluator<OoANavGen2bitAgent>(this, pCG, NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("NPPWeight", pSECap));

    m_pME = new MultiEvaluator<OoANavGen2bitAgent>(this, m_pCG, m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true); // true: delete evaluators

    OoANavGen2bitAgent agent;
    m_pVerVarK = new VerhulstVarK<OoANavGen2bitAgent>(this, m_pCG, m_apWELL, m_adCapacities, iCapacityStride, (int)qoffsetof(agent, m_iMateIndex));


    // evaluator for movement

    m_pWM = new WeightedMove<OoANavGen2bitAgent>(this, m_pCG, m_apWELL, m_adEnvWeights);

    m_pPair = new RandomPair<OoANavGen2bitAgent>(this, m_pCG, m_apWELL);

    m_pGO = new GetOld<OoANavGen2bitAgent>(this, m_pCG);

    m_pOAD = new OldAgeDeath<OoANavGen2bitAgent>(this, m_pCG, m_apWELL);

    m_pFert = new Fertility<OoANavGen2bitAgent>(this, m_pCG);

    m_pGenetics = new Genetics<OoANavGen2bitAgent,GeneUtils>(this, m_pCG, m_pAgentController, &m_vMergedDeadList, m_aiSeeds[1]);

    m_pNavigate = new Navigate<OoANavGen2bitAgent>(this, m_pCG, m_apWELL);

    //    m_pCM = new ConfinedMove<OoANavGen2bitAgent>(this, m_pCG, this->m_vMoveList);
...
// destructor
//
OoANavGen2bitPop::~OoANavGen2bitPop() {
...
// setParams
//
int OoANavGen2bitPop::setParams(const char *pParams) {
    int iResult = 0;
...
// preLoop
//
int OoANavGen2bitPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavGen2bitAgent>::preLoop();
...
//
int OoANavGen2bitPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavGen2bitPop::flushEvents(float fT) {
...
//
int OoANavGen2bitPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
...
//
int OoANavGen2bitPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGen2bitPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void OoANavGen2bitPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {
...
//
int OoANavGen2bitPop::readAdditionalDataQDF(hid_t hSpeciesGroup) {
