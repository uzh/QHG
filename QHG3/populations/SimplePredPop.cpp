...
// constructor
//
SimplePredPop::SimplePredPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<SimplePredAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
      m_pWM(NULL),
      m_pSE(NULL),
      m_pMM(NULL),
      m_pBB(NULL),
    ...
    m_pSE = new ShareEvaluator<SimplePredAgent>(this, m_pCG, m_adEnvWeights, "PreyMass",  true, EVENT_ID_NONE);
...
#pragma omp parallel 
    ...
    m_prio.addAction(ATTR_HUNTING_NAME, m_pHU);
