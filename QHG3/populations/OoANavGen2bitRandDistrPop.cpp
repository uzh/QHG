...
#include "OoANavGen2bitRandDistrPop.h"


//----------------------------------------------------------------------------
// constructor
//
OoANavGen2bitRandDistrPop::OoANavGen2bitRandDistrPop(SCellGrid *pCG, PopFinder *pPopFinder, int iLayerSize, IDGen **apIDG, uint32_t *aulState, uint *aiSeeds) 
    : SPopulation<OoANavGen2bitRandDistrAgent>(pCG, pPopFinder, iLayerSize, apIDG, aulState, aiSeeds),
    ...
    m_pNPPCap = new NPPCapacity<OoANavGen2bitRandDistrAgent>(this, pCG, m_apWELL, m_adCapacities, iCapacityStride); 
    ...
    SingleEvaluator<OoANavGen2bitRandDistrAgent> *pSEAlt = new SingleEvaluator<OoANavGen2bitRandDistrAgent>(this, pCG, NULL, (double*)m_pCG->m_pGeography->m_adAltitude, "AltCapPref", true, EVENT_ID_GEO);
    ...
    SingleEvaluator<OoANavGen2bitRandDistrAgent> *pSECap = new SingleEvaluator<OoANavGen2bitRandDistrAgent>(this, pCG, NULL, m_adCapacities, "NPPPref", true, EVENT_ID_VEG);
    mEvalInfo.push_back(std::pair<std::string, Evaluator*>("NPPWeight", pSECap));

    m_pME = new MultiEvaluator<OoANavGen2bitRandDistrAgent>(this, m_pCG, m_adEnvWeights, mEvalInfo, MODE_ADD_SIMPLE, true);  // true: delete evaluators

    OoANavGen2bitRandDistrAgent agent;
    m_pVerVarK = new VerhulstVarK<OoANavGen2bitRandDistrAgent>(this, m_pCG, m_apWELL, m_adCapacities, iCapacityStride, (int)qoffsetof(agent, m_iMateIndex));


    // evaluator for movement

    m_pWM = new WeightedMove<OoANavGen2bitRandDistrAgent>(this, m_pCG, m_apWELL, m_adEnvWeights);


    m_pGO = new GetOld<OoANavGen2bitRandDistrAgent>(this, m_pCG);

    m_pOAD = new OldAgeDeath<OoANavGen2bitRandDistrAgent>(this, m_pCG, m_apWELL);

    m_pFert = new Fertility<OoANavGen2bitRandDistrAgent>(this, m_pCG);

    m_pGenetics = new Genetics<OoANavGen2bitRandDistrAgent,GeneUtils>(this, m_pCG, m_pAgentController, &m_vMergedDeadList, m_aiSeeds[1]);

    m_pPair = new RandDistrPair<OoANavGen2bitRandDistrAgent, GeneUtils>(this, m_pCG, m_pGenetics, m_apWELL);

    m_pNavigate = new Navigate<OoANavGen2bitRandDistrAgent>(this, m_pCG, m_apWELL);

    //    m_pCM = new ConfinedMove<OoANavGen2bitRandDistrAgent>(this, m_pCG, this->m_vMoveList);
    ...
    m_prio.addAction(ATTR_RANDDISTRPAIR_NAME, m_pPair);
...
// destructor
//
OoANavGen2bitRandDistrPop::~OoANavGen2bitRandDistrPop() {
...
// setParams
//
int OoANavGen2bitRandDistrPop::setParams(const char *pParams) {
    int iResult = 0;
...
// preLoop
//
int OoANavGen2bitRandDistrPop::preLoop() {
    // here we should createGenomes (if this is a run started from scratch)
    int iResult = SPopulation<OoANavGen2bitRandDistrAgent>::preLoop();
...
//
int OoANavGen2bitRandDistrPop::updateEvent(int iEventID, char *pData, float fT) {
...
// flushEvents
//
void OoANavGen2bitRandDistrPop::flushEvents(float fT) {
...
//
int OoANavGen2bitRandDistrPop::makePopSpecificOffspring(int iAgent, int iMother, int iFather) {
    int iResult = 0;    
...
//
int OoANavGen2bitRandDistrPop::writeAdditionalDataQDF(hid_t hSpeciesGroup) {
...
//
int OoANavGen2bitRandDistrPop::addPopSpecificAgentData(int iAgentIndex, char **ppData) {
    int iResult = 0;
    if (iResult == 0) {
...
//
void OoANavGen2bitRandDistrPop::addPopSpecificAgentDataTypeQDF(hid_t *hAgentDataType) {
...
//
int OoANavGen2bitRandDistrPop::readAdditionalDataQDF(hid_t hSpeciesGroup) {
