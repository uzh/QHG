...
void usage(const char *pApp) {
    ...
    printf("  subdivtiles  subdivision of tiles (use 0 or omit for an untiled grid)\n");
...
int doIcoSplit(int iSubDivNodes, char *sSubDivTiles, char *sQDFBody, char *sIGNBody, int iVerbosity) {
    ...
    if (sSubDivTiles == NULL) {
        iSubDivTiles = 0;
        iResult = 0;
    } else  if (strToNum(sSubDivTiles, &iSubDivTiles)) {
        iResult = 0;
    }
    if (iResult == 0) {
        if ((iSubDivNodes > 0) && (iSubDivTiles >= 0)) {
            ...
            if (iResult == 0) {
                printf("Creating EQTiling instance\n");
